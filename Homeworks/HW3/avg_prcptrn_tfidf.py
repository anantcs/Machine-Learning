...
def model_counts(data, test_data):
    ...
    transformer = TfidfTransformer(smooth_idf=False, use_idf = True, norm=None)
    X_train_counts = count_vect.fit_transform(data['text'])
    X_test_counts = count_vect.transform(test_data['text'])
    X_train_tfidf = transformer.fit_transform(X_train_counts)
    X_test_tfidf = transformer.fit_transform(X_test_counts)
    temp1 = (X_train_tfidf != 0)
    temp2 = (X_test_tfidf != 0)
    X_train_tfidf[temp1] -= 1
    X_test_tfidf[temp2] -= 1
    X_train_tfidf /= math.log(10)
    X_test_tfidf /= math.log(10)
...
def main():
    ...
    training_size, test_size = 100000, 2000
    ...
    print 'The average accuracy rate is %s %%' % np.mean(predictions)
